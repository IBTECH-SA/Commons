...
import subscriber.common.CommonConstants;
import subscriber.common.MessageProcessor;
 ...
 */
public class Subscriber extends SubscriberBase implements nEventListener {
	...
	private nChannel channelToSubscribe;
	...
	public static void main(String[] args) {
		...
		String channelName = configProperties.getProperty(CommonConstants.TOPIC_NAME);

		startEid = -1; // Default value (points to last event in the channel +

		int loglvl = 1; // Default value
		loglvl = Integer.parseInt(configProperties.getProperty(CommonConstants.LOGLEVEL));
		...
		String RNAME = configProperties.getProperty(CommonConstants.RNAME);
		...
		mySelf.subscribeToChannel(rproperties, channelName, selector, startEid, loglvl, reportCount);
	}

	/**
	 * This method demonstrates the Nirvana API calls necessary to subscribe to a
	 * channel. It is called after all command line arguments have been received and
	 * validated
	 *
	 ...
	 */
	private void subscribeToChannel(String[] realmDetails, String achannelName, String selector, long startEid,
			int loglvl, int repCount) {
		logLevel = loglvl;
		reportCount = repCount;

		mySelf.constructSession(realmDetails, configProperties);

		// Subscribes to the specified channel
		try {

			// Create a channel attributes object
			nChannelAttributes nca = new nChannelAttributes(achannelName);

			// Obtain the channel reference
			channelToSubscribe = mySession.findChannel(nca);

			// if the latest event has been implied (by specifying -1)
			if (startEid == -1) {
				// Get the last eid on the channel and reset the start eid with
				// that value
				startEid = channelToSubscribe.getLastEID();
			}

			// Add this object as a subscribe to the channel with the specified
			// message selector channel
			// and start eid

			System.out.println("Subscribing to channel:" + achannelName);
			channelToSubscribe.addSubscriber(this, selector, startEid);

			// Stay subscribed until the user presses any key
			System.out.println("Subscribed successfully. Press any key to disconnect !");
			...
			channelToSubscribe.removeSubscriber(this);
		}
		// Handle errors
	 ...
	 * processing the message inside this method, as until it exits the next message
	 * can not be pushed.
	 *
	 * @param evt An nConsumeEvent object containing the message received from the
	 *            channel
	 */
	public void go(nConsumeEvent evt) {
		...
		if (count == reportCount) {
			...
			if (logLevel >= 1) {
				...
				if (end != startTime) {
					System.out.println("Received " + reportCount + " in " + (end - startTime) + " Evt/Sec = "
							+ ((reportCount * 1000) / (end - startTime)) + " Bytes/sec="
							+ ((byteCount * 1000) / (end - startTime)));
					System.out.println("Bandwidth data : Bytes Tx [" + mySession.getOutputByteCount() + "] Bytes Rx ["
